...
void main() {
    ...
    float alpha = glow * 0.34 + softHalo + nucleus * 0.82 + shimmer * 0.14;
    alpha *= smoothstep(0.08, 0.34, length(vObjectPosition));

    if (alpha < 0.015) discard;

    vec3 nucleusColor = vec3(0.96, 0.98, 1.0);
    vec3 color = vColor * (0.34 + glow * 1.08 + softHalo * 0.48) + nucleusColor * (nucleus * 1.55 + shimmer * 0.58);
