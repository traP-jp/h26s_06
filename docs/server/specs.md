...
`Score` は固定上限を持たない相対値として扱い、init および sync 配信時は小数第3位に丸める。投稿は対象チャンネルに `messageScoreAmount * log(文字数)`、閲覧移動は `0.25` を加算し、親チャンネルへは階層ごとに `0.45` 倍して伝播する。`trigger` SSE イベントの `delta` には対象チャンネル（深さ0）へ加算した値を入れる。減衰は指数減衰で、時定数は約300秒とする。
