...
```json
event: trigger
data: {"type": "msg", "ch": "sub_ch_10", "delta": 2.303}
...
`delta` は対象チャンネル（深さ0）に加算されるスコアの増加量。投稿時は `messageScoreAmount * log(文字数)`、移動時は固定値を用いる。祖先チャンネルへの伝播は `ancestorScoreFactor` の累乗で減衰する。
